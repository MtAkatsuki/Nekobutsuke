...
// 4x4 Bayer ディザリング閾値：戻り値は [0,1)。
// clip() と組み合わせることで、スクリーンドア（ディザ）による透明表現を実現。
// ToonPS / OutlinePS で共通利用
float DitherThreshold(float2 svpos)
{
    ...
    int2 pix = (int2) svpos; // スクリーン上のピクセル座標
    return bayer[(pix.y & 3) * 4 + (pix.x & 3)];
}